    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            try:
                ...
                # Pass 1: Calculate global baselines with minimal memory
                df_for_baseline = pd.read_csv(filepath, usecols=['Amount Paid', 'Account'])
                df_for_baseline.rename(columns={'Amount Paid': 'amount', 'Account': 'from_account'}, inplace=True)
                df_for_baseline['amount'] = pd.to_numeric(df_for_baseline['amount'], errors='coerce').fillna(0)
                customer_baselines = df_for_baseline.groupby('from_account')['amount'].agg(['mean', 'std']).fillna(0)
                del df_for_baseline

                # Pass 2: Process file in chunks for memory safety
                chunk_size = 10000
                suspicious_alerts = []
                total_volume = 0
                ...
                for chunk in pd.read_csv(filepath, chunksize=chunk_size, header=0, names=new_cols, skiprows=1):
                    ...
                    chunk['risk_level'] = pd.cut(chunk['risk_score'], bins=[-1, 40, 70, 101], labels=['Low', 'Medium', 'High'])
                    suspicious_df = chunk[chunk['risk_score'] >= 50]
